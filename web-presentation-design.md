...
- **准备演示图片 (可选与规范)**：如果汇报中需要展示超高清原图，请将图片直接存放在与 `index.html` 同级或项目根目录下。单图可以直接命名为对应页码（如 `6.png`）；同页多图请添加序号（如 `6.png`, `6.1.png`, `6.2.png`）。
- **AI 自动探测与重命名机制 (Agent 必做)**：**如果用户提供的图片没有按规范命名也没关系。Agent 在接到任务时，必须主动扫描根目录和 `web` 目录下的可用图片，根据用户的展示意图，主动将这些图片复制到 `web` 目录下，并重命名为规范的数字序号（如 `6.png`, `6.1.png`），然后再在 JS 的 `slideImages` 中使用这些规范的序号名称**。
- **触发机制与无缝轮换**：放映时按键盘 **向下方向键 (↓)** 即可呼出图片并无缝轮换。
- **静默防尴尬**：如果当前页没有图片，按了 ↓ 键也**绝对静默，不触发任何操作或报错**，保障汇报的绝对流畅。
